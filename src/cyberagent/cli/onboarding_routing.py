...
def seed_procedure_routing_rules(
    team_id: int, procedure_templates: list[dict[str, object]]
) -> None:
    system3 = systems_service.get_system_by_type(team_id, SystemType.CONTROL)
    existing = {
        rule.name
        for rule in routing_service.list_routing_rules(team_id, active_only=False)
    }
    for template in procedure_templates:
        if not isinstance(template, dict):
            continue
        rules = template.get("routing_rules")
        if not isinstance(rules, list):
            continue
        for rule in rules:
            if not isinstance(rule, dict):
            ...
            routing_service.create_routing_rule(
                ...
                priority=int(priority) if isinstance(priority, int) else 0,
                active=bool(active) if isinstance(active, bool) else True,
                created_by_system_id=system3.id if system3 else None,
            )
